...
const ProductDetails = () => {
  ...
  return (
    <div className='container mx-auto p-4'>
        <div className='min-h-[200px] flex flex-col md:flex-row gap-4 items-center'>
            <div className='h-96 flex flex-col md:flex-row-reverse gap-4'>
                <div className='h-[300px] w-[300px] md:h-96, md:w-96 bg-slate-200 relative p-2 sm:h-full sm:w-full'>
                    <img src={activeImage} className='h-full w-full object-scale-down mix-blend-multiply' onMouseMove={handleZoomImage} onMouseLeave={handleLeaveImageZoom}/>

                    {
                        zoomImage && (
                            <div className='hidden md:block absolute min-w-[500px] overflow-hidden min-h-[400px] bg-slate-200 p-1 -right-[510px] top-0'>
...

            {
                loading ? (
                    <div className='grid gap-1 w-full'>
                        <p className='bg-slate-200 animate-pulse h-6 md:h-8 w-full rounded-full inline-block'></p>
                        <h2 className='text-2xl md:text-4xl font-medium h-6 md:h-8 bg-slate-200 animate-pulse w-full'></h2>
                        <p className='capitalize text-slate-400 bg-slate-200 min-w-[100px] animate-pulse h-6 md:h-8 w-full'></p>

                        <div className='text-red-600 bg-slate-200 h-6 md:h-8 animate-pulse flex items-center gap-1 w-full'></div>

                        <div className='flex items-center gap-2 text-2xl md:text-3xl font-medium my-1 h-6 md:h-8 animate-pulse w-full'>
                            <p className='text-red-600 bg-slate-200 w-full'></p>
                            <p className='text-slate-400 line-through bg-slate-200 w-full'></p>
                        </div>

                        <div className='flex items-center gap-3 my-2 w-full'>
                            <button className='h-6 md:h-8 bg-slate-200 rounded animate-pulse w-full'></button>
                            <button className='h-6 md:h-8 bg-slate-200 rounded animate-pulse w-full'></button>
                        </div>

                        <div className='w-full'>
                            <p className='text-slate-600 font-medium my-1 h-6 md:h-8   bg-slate-200 rounded animate-pulse w-full'></p>
                            <p className=' bg-slate-200 rounded animate-pulse h-10 md:h-12  w-full'></p>
                        </div>
                    </div>
                ) : (
